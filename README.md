...
- **多标签 SSH**：双击/单击资产即新建一个 SSH 终端标签，可同时连接多台服务器。
...
1. 打开应用，在登录表单中填写：
   ...
   - **Key ID**：访问密钥 ID
   - **Secret**：访问密钥
2. 点击 **连接 JumpServer**，验证成功后左侧会加载资产列表并显示当前登录用户。
3. 单击/双击资产列表中的服务器，会在右侧新建一个 SSH 终端标签并自动连接。
4. 使用标签栏的左右滚动按钮、鼠标滚轮或按住空白处拖动来滚动标签；支持关闭、切换、拖拽排序。
5. 在左侧顶部可切换终端配色方案、深色/亮色主题，以及刷新资产列表。
6. 在左侧「快捷指令」区域可新增指令；点击指令发送到当前终端，点击广播按钮发送到所有已连接终端。
7. `Ctrl/Cmd + F` 打开终端内容搜索栏，支持下一个/上一个、区分大小写。
8. 拖动左侧侧边栏分隔条可调整宽度，终端会自动 fit 并同步远端 PTY。
